...
UDEV_CHANGED=0
_install_file "$WORK_DIR/90-ghelper.rules" "$UDEV_DEST" 644 "udev rules" && UDEV_CHANGED=0 || UDEV_CHANGED=1
...
_install_file "$WORK_DIR/ghelper.desktop" "$DESKTOP_DEST" 644 "desktop entry" || true
...
if [[ -n "$REAL_USER" ]]; then
    ...
    if [[ -f "$AUTOSTART_DEST" ]] && cmp -s "$WORK_DIR/ghelper.desktop" "$AUTOSTART_DEST"; then
